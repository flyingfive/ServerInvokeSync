...
namespace CSock.DynamicProxy
{
    ...
    public class ProxyObjectFactory
    {
        ...
        private const string DYNAMIC_ASSEMBLY_NAME = "LocalDynamicProxies";
        ...
        private static void GenerateMethod(Type interfaceType, FieldBuilder handlerField, TypeBuilder typeBuilder)
        {
            ...
            foreach (var methodInfo in interfaceMethods)
            {
                ...
                iLGenerator.Emit(OpCodes.Callvirt, _handlerInvokeMethod);//typeof(IProxyInvocationHandler).GetMethod("Invoke"));
        ...
        /// <param name="client">需要调用的客户端业务标识</param>
        /// <returns></returns>
        public T CreateInterfaceProxyWithoutTarget<T>(SockClient client) where T : class
        {
            ...
            var handler = new SocketInvocationHandler(client);
